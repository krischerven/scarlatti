...
from scarlatti.utils import emit_signal, get_network_available
from scarlatti.helper_web_spotify import SpotifyWebHelper
from scarlatti.helper_web_save import SaveWebHelper
from scarlatti.define import App, StorageType
        ...
        try:
            ...
            if status:
                ...
                for album in decode["albums"]["items"]:
                    payload = self.scarlatti_album_payload(album)
                    self.save_album_payload_to_db(payload,
                                                  storage_type,
                                                  True,
                                                  cancellable)
                for track in decode["tracks"]["items"]:
                    payload = self.scarlatti_album_payload(track["album"])
                    item = self.save_album_payload_to_db(
                                                  payload,
                                                  storage_type,
                                                  False,
                                                  cancellable)
                    payload = self.scarlatti_track_payload(track)
    ...
    def load_tracks(self, album, cancellable):
        ...
        try:
            ...
            if status:
                ...
                for track in decode["tracks"]["items"]:
                    scarlatti_payload = self.scarlatti_track_payload(track)
                    self.save_track_payload_to_db(scarlatti_payload,
                                                  album_item,
                                                  album.storage_type,
                                                  False,
                                                  cancellable)
