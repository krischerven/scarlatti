...
from scarlatti.define import StorageType, App
from scarlatti.utils import emit_signal, get_network_available
from scarlatti.search_local import LocalSearch
...
class Search(GObject.Object):
    ...
    def set_web_search(self, name):
        ...
        if name == "SPOTIFY":
            from scarlatti.search_spotify import SpotifySearch
            self.__web_search = SpotifySearch()
            self.__connect_search_signals(self.__web_search)
        elif name == "LASTFM":
            from scarlatti.search_lastfm import LastFMSearch
            self.__web_search = LastFMSearch()
            self.__connect_search_signals(self.__web_search)
        elif name == "MUSICBRAINZ":
            from scarlatti.search_musicbrainz import MusicBrainzSearch
            self.__web_search = MusicBrainzSearch()
            self.__connect_search_signals(self.__web_search)
        elif name == "DEEZER":
            from scarlatti.search_deezer import DeezerSearch
            self.__web_search = DeezerSearch()
            self.__connect_search_signals(self.__web_search)
        elif name == "JAMENDO":
            from scarlatti.search_jamendo import JamendoSearch
