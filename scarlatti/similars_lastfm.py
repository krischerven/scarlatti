...
from scarlatti.define import LASTFM_API_KEY, App
from scarlatti.logger import Logger
from scarlatti.utils import emit_signal, get_network_available
from scarlatti.helper_web_lastfm import LastFMWebHelper
    ...
    def load_similars(self, artist_ids, storage_type, cancellable):
        ...
        for (artist_name, cover_uri) in result:
            ...
            for (album, artist) in albums:
                ...
                if payload is None:
                    continue
                scarlatti_payload = self.scarlatti_album_payload(payload)
                item = self.save_album_payload_to_db(scarlatti_payload,
                                                     storage_type,
                                                     True,
                                                     cancellable)
                ...
                i = 1
                for track in tracks:
                    scarlatti_payload = self.scarlatti_track_payload(track, i)
                    i += 1
