...
from scarlatti.player_plugins import PluginsPlayer
from scarlatti.define import GstPlayFlags, App, StorageType, Repeat
from scarlatti.codecs import Codecs
...
class BinPlayer:
    ...
    def stop_all(self):
        """
            Stop all bins, scarlatti should quit now
        """
        # Stop
    ...
    def _on_bus_error(self, bus, message):
        ...
        if self.current_track.id is not None and self.current_track.id >= 0:
            if self.__codecs.is_missing_codec(message):
                ...
                App().notify.send("Scarlatti", parsed)
                self.stop()
